...
void ntyco_client_co(void *arg) {
    ...
    while (1) {
        ...
        // printf("[DEBUG] g_slave_fd=%d, g_enable_repl_master=%d, g_use_tcp_sync=%d, g_sync_file_done=%d\n",g_slave_fd, g_enable_repl_master, g_use_tcp_sync, g_sync_file_done);
        
        if (g_slave_fd > 0 && g_enable_repl_master && g_use_tcp_sync && g_sync_file_done) {
            // 如果直接send发送 只要进入这个分支 客户端向主端插入数据就会失败 怎么解决？？？       
            send(g_slave_fd, c->rbuffer + c->rlength - count, count, MSG_DONTWAIT);
        }
