...
int keep_add_keyword_index(const char *keyword, const char *key){
    if (keyword == NULL || key == NULL) {
        printf("[KEEP] invalid keyword/key\n");
        return -1;
    }
    if (keyword_index_count >= MAX_KEYWORD_INDEX) {
        printf("[KEEP] keyword index is full\n");
        return -1;
    }
...
int keep_add_vector_index(const char *key, const float *vector, size_t dim){
    if (key == NULL || vector == NULL || dim == 0) {
        printf("[KEEP] invalid vector parameters\n");
        return -1;
    }
    if (global_vector_index_count >= MAX_VECTOR_INDEX) {
        printf("[KEEP] vector index is full\n");
        return -1;
    }
    ...
    if (entry->vector == NULL) {
        printf("[KEEP] vector kvs_malloc failed\n");
        return -1;
    }
    memcpy(entry->vector, vector, sizeof(float) * dim);
    entry->dim = dim;
    printf("[KEEP] vector index: key=%s dim=%zu\n", key, dim);
...
int keep_build_index(const char *key, const char *value, size_t value_len){
    if (key == NULL || value == NULL) {
        printf("[KEEP] invalid key/value\n");
        return -1;
    }
    printf("[KEEP] build index: key=%s value_len=%zu\n", key, value_len);
    char *json = kvs_malloc(value_len + 1);
    if (json == NULL) {
        printf("[KEEP] JSON buffer kvs_malloc failed\n");
        return -1;
    }
    ...
    if (keywords != NULL && cJSON_IsArray(keywords)) {
        ...
        printf("[KEEP] keyword count=%d\n", count);
        for (int i = 0; i < count; i++) {
            cJSON *item = cJSON_GetArrayItem(keywords, i);
            if (!cJSON_IsString(item)) continue;
            printf("[KEEP] keyword[%d]=%s\n", i, item->valuestring);
    ...
    cJSON_Delete(root);
    printf("[KEEP] build index success: %s\n", key);
    return 0;
}
...
int ai_text_to_vector(const char *text, float *vector, size_t dim){
    ...
    if (request == NULL) {
        printf("[EMBEDDING] create request failed\n");
        return -1;
    }
    ...
    if (request_body == NULL) {
        printf("[EMBEDDING] create request JSON failed\n");
        return -1;
    }
    printf("[EMBEDDING] request: %s\n", request_body);

    CURL *curl = curl_easy_init();
    if (curl == NULL) {
        printf("[EMBEDDING] curl init failed\n");
        kvs_free(request_body);
        return -1;
    }
    ...
    printf("[EMBEDDING] HTTP status=%ld\n", http_code);
    curl_slist_free_all(headers);
    curl_easy_cleanup(curl);
    kvs_free(request_body);

    if (curl_ret != CURLE_OK) {
        printf("[EMBEDDING] curl failed: %s\n", curl_easy_strerror(curl_ret));
        kvs_free(response.data);
        return -1;
    }
    if (http_code < 200 || http_code >= 300) {
        printf("[EMBEDDING] HTTP error: %ld\n", http_code);
    ...
    if (response.data == NULL) {
        printf("[EMBEDDING] empty response\n");
        return -1;
    }
    printf("[EMBEDDING] response size=%zu\n", response.size);

    cJSON *root = cJSON_Parse(response.data);
    kvs_free(response.data);
    if (root == NULL) {
        printf("[EMBEDDING] response JSON parse failed\n");
        return -1;
    }

    cJSON *dimension = cJSON_GetObjectItem(root, "dimension");
    if (dimension == NULL || !cJSON_IsNumber(dimension)) {
        printf("[EMBEDDING] dimension not found\n");
        cJSON_Delete(root);
        return -1;
    }
    int embedding_dim = dimension->valueint;
    printf("[EMBEDDING] returned dimension=%d\n", embedding_dim);
    if (embedding_dim != (int)dim) {
        printf("[EMBEDDING] dimension mismatch: actual=%d expected=%zu\n", embedding_dim, dim);
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (embedding == NULL || !cJSON_IsArray(embedding)) {
        printf("[EMBEDDING] embedding not found\n");
        cJSON_Delete(root);
        return -1;
    }
    int array_size = cJSON_GetArraySize(embedding);
    if (array_size != (int)dim) {
        printf("[EMBEDDING] embedding array size mismatch: actual=%d expected=%zu\n", array_size, dim);
        cJSON_Delete(root);
        return -1;
    }
    ...
    for (size_t i = 0; i < dim; i++) {
        ...
        if (item == NULL || !cJSON_IsNumber(item)) {
            printf("[EMBEDDING] invalid embedding value at index=%zu\n", i);
            cJSON_Delete(root);
            return -1;
        }
    ...
    cJSON_Delete(root);
    printf("[EMBEDDING] vector generated successfully: dim=%zu\n", dim);
    return 0;
}
...
void match_set_threshold(float threshold){
    ...
    printf("[MATCH] similarity threshold = %.4f\n", g_match_threshold);
}
...
/* 判断问题是否包含关键词 */
static int question_contains_keyword(
    const char *question,
    const char *keyword)
{
...
static size_t match_find_keyword_candidates(const char *question, char **candidates, size_t max_candidates){
    ...
    for (size_t i = 0; i < keyword_index_count; i++) {
        ...
        printf("[MATCH] keyword candidate: %s -> %s\n", keyword, key);
        count++;
    }
    printf("[MATCH] keyword candidates=%zu\n", count);
    return count;
}
...
int match_find_best_key(const char *question, char *best_key, size_t best_key_size, float *best_score){
    ...
    if (global_vector_index_count == 0) {
        printf("[MATCH] vector index empty\n");
        return 1;
    }

    float question_vector[MATCH_VECTOR_DIM];
    if (ai_text_to_vector(question, question_vector, MATCH_VECTOR_DIM) != 0) {
        printf("[MATCH] question vector failed\n");
        return -1;
    }

    char **candidates = kvs_malloc(sizeof(char *) * MAX_KEYWORD_INDEX);
    if (candidates == NULL) {
        printf("[MATCH] candidates malloc failed\n");
        return -1;
    }
    size_t allocated = 0;
    ...
    if (use_all_vectors) {
        printf("[MATCH] no keyword candidate, scan all vector index\n");
    } else {
        printf("[MATCH] use keyword candidates\n");
    }
    ...
    for (size_t i = 0; i < global_vector_index_count; i++) {
        ...
        if (entry->dim != MATCH_VECTOR_DIM) {
            printf("[MATCH] skip key=%s dim=%zu expected=%d\n", entry->key, entry->dim, MATCH_VECTOR_DIM);
            continue;
        }
        float score = ai_cosine_similarity(question_vector, entry->vector, MATCH_VECTOR_DIM);
        if (score < 0.0f) continue;
        printf("[MATCH] key=%s similarity=%.6f\n", entry->key, score);
    ...
    if (max_key == NULL) {
        printf("[MATCH] no vector candidate\n");
        return 1;
    }

    printf("[MATCH] best key=%s score=%.6f threshold=%.6f\n", max_key, max_score, g_match_threshold);
    if (max_score < g_match_threshold) {
        printf("[MATCH] similarity below threshold\n");
        return 1;
    }
    ...
    printf("[MATCH] MATCH SUCCESS: key=%s score=%.6f\n", best_key, max_score);
    return 0;
}
...
int match_find_answer(const char *question, char **answer, size_t *answer_len, float *best_score){
    ...
    *answer_len = 0;
    ...
    if (result == NULL || result->data == NULL || result->len == 0) {
        printf("[MATCH] hash record not found: %s\n", best_key);
        return -1;
    }

    char *json = kvs_malloc(result->len + 1);
    if (json == NULL) {
        printf("[MATCH] JSON kvs_malloc failed\n");
        return -1;
    }
    ...
    *answer_len = len;
    ...
    printf("[MATCH] answer found: key=%s score=%.6f answer_len=%zu\n", best_key, score, len);
    cJSON_Delete(root);
    return 0;
}
